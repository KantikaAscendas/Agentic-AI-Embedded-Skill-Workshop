...
releaseName = string(release.Release);
releaseParts = regexp(releaseName, "^R(\d{4})([ab])$", "tokens", "once");

if isempty(releaseParts)
    versionValidity = false;
    versionFailureMessage = "Unable to determine the MATLAB release.";
    return;
end

releaseKey = 2 * str2double(releaseParts{1}) + double(releaseParts{2} == "b");
minimumReleaseKey = 2 * 2025 + 1; % R2025b
versionValidity = releaseKey >= minimumReleaseKey;

if versionValidity
    versionFailureMessage = "";
else
    versionFailureMessage = "MATLAB R2025b or newer is required. R2026a is the latest fully tested release.";
end
return
end
